...
$footerRole = (string) ($_SESSION['user_role'] ?? 'guest');
?>
</main>
<footer class="site-footer">
    <div class="container-fluid px-3 px-lg-4 d-flex flex-column flex-md-row gap-2 justify-content-between">
        ...
        <small>
            <a href="<?= $baseHref ?>/pages/campaigns.php"><?= htmlspecialchars(sisonke_t('nav_marketplace'), ENT_QUOTES, 'UTF-8') ?></a>
            <?php if ($footerRole === 'admin'): ?>
                <span aria-hidden="true"> / </span>
                <a href="<?= $baseHref ?>/admin/dashboard.php"><?= htmlspecialchars(sisonke_t('nav_admin'), ENT_QUOTES, 'UTF-8') ?></a>
            <?php endif; ?>
        </small>
    </div>
</footer>
